...
myMidiOutMessage( HMIDIOUT hMidi, int iStatus, int iChannel, int iData1,  int iData2 )
{
    ...
    wsprintf(szBuffer,"0x%02X 0x%02X 0x%02X", (iStatus|iChannel), iData1, iData2 );
    SetWindowText( SomeCtrlGetHWND(SOME_CTRL_MIDI_MESSAGE), szBuffer );

    dwMessage = (iData2 << 16) | (iData1 << 8) | iStatus | iChannel ;
 ...
 **********************************************/
BOOL
MidiOutData( BYTE channel, BYTE status, BYTE data1, BYTE data2 )
{
    MMRESULT result;
    DWORD dwMessage;
    ...
    if( midiInfo.hMidiOut )
    {
        result = myMidiOutMessage(midiInfo.hMidiOut,status,channel,data1,data2);
    }
    else
    {
        nop();
 ...
 **********************************************/
BOOL
MidiNoteOn( INT channel, INT noteNum, INT velocity )
{
    MMRESULT result;
    DWORD dwMessage;
...
//        result = midiOutShortMsg(midiInfo.hMidiOut,dwMessage);
        result = myMidiOutMessage(midiInfo.hMidiOut,0x90,channel,noteNum,velocity);
    }
    else
    {
        nop();
 ...
 **********************************************/
BOOL
MidiNoteOff( INT channel, INT noteNum )
{
    MMRESULT result;
    DWORD dwMessage;
    ...
    {
//        result = midiOutShortMsg(midiInfo.hMidiOut,dwMessage);
        result = myMidiOutMessage(midiInfo.hMidiOut,0x80,channel,noteNum,0);
    }
    else
    {
        nop();
